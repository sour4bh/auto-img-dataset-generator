...
def arg_parse():
	...
	parser.add_argument('--output', help='output file root', 
							default='results', type=str)
	parser.add_argument('--input', help='the name of the file which constain the id', 
							default='imglist.txt', type=str)
...
if __name__ == '__main__':
	args = arg_parse()
	start = time.time()
	assert args.input != None, "Name list cannot be None!"
	# get all id as type of list of str
	nameList = list(set(readFile(args.input)))
	...
	if isWindows:
		...
		chromedriver = os.path.join(os.getcwd(),'driver/chromedriver')
	...
	gdownloader = GoogleDownloader(nameList = nameList, root = args.output, size = args.size, 
